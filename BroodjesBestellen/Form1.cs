...
namespace BroodjesBestellen
{
    public partial class FrmBroodjes : Form
    {
        ...
        private void TxtHoeveelheid_TextChanged(object sender, EventArgs e)
        {
            if(TxtPrijs.Text!="" || TxtHoeveelheid.Text != "")
            {
                try
                {
                    int totaal = Convert.ToInt32(TxtHoeveelheid.Text);
                    double prijs = Convert.ToDouble(TxtPrijs.Text);
                    txtTotaal.Text = Convert.ToString(totaal * prijs);
                }
                catch { }
            }
            else
